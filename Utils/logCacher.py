...
class LocalCache:
    ...
    def reload(self, filename, filterStr):
        ...
        with open(filename, "r", encoding="utf-8", errors="ignore") as file:
            for line in file:
                if filterStr is not None:
                    if line.find(filterStr) == -1:  # not found this filter in this line
                        continue
                self.__numLines += 1
                ...
                if self.__numLines % self.__MAX_LINES_PER_PAGE == 0:
                    ...
                    tempLines.clear()
        if self.__numLines < self.__MAX_LINES_PER_PAGE:
            self.__cachePages.append(''.join(unidecode(each) for each in tempLines))
            tempLines.clear()
    ...
    def get_partial_block(self, pages):
        if pages >= 0:
            ...
            print('appending all pages finished: \n\t pages from(0 ~ %d) \n\t total lines:%d' % (
                self.__currentPageIdx, self.__numLines))
            res = ''.join(each for each in self.__cachePages)
        return res
